...
function PasswordStrengthBar({ password }: { password: string }) {
  ...
  const textColors = ['', 'text-danger-400', 'text-warning-400', 'text-primary-400', 'text-success-400']
    ...
    <div className="flex flex-col gap-1.5 mt-1">
      <div className="flex gap-1">
        {[1, 2, 3, 4].map(i => (
          <div
            key={i}
            className={['h-1 flex-1 rounded-full transition-all duration-300', i <= strength ? colors[strength] : 'bg-neutral-800'].join(' ')}
          />
        ))}
      </div>
      {strength > 0 && <p className={`text-xs ${textColors[strength]}`}>{labels[strength]}</p>}
    </div>
  )
}
...
export default function SignupPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const message =
        err instanceof Error && err.message ? err.message : 'Failed to create account. Please try again.'
      setError(message)
    }
  }
        ...
        <form onSubmit={(e) => { void handleSubmit(e) }} noValidate className="flex flex-col gap-5">
          ...
          <div>
            <Input
              ...
              rightAddon={
                <button type="button" onClick={() => { setShowPassword(v => !v) }} aria-label={showPassword ? 'Hide password' : 'Show password'} className="text-neutral-500 hover:text-neutral-300 transition-colors">
          ...
          <Input
            ...
            leftAddon={
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="w-4 h-4">
                <path fillRule="evenodd" d="M10 1a4.5 4.5 0 00-4.5 4.5V9H5a2 2 0 002-2v-6a2 2 0 002-2h-.5V5.5A4.5 4.5 0 0010 1zm3 8V5.5a3 3 0 10-6 0V9h6z" clipRule="evenodd" />
              </svg>
            }
            rightAddon={
              <button type="button" onClick={() => { setShowConfirm(v => !v) }} aria-label={showConfirm ? 'Hide password' : 'Show password'} className="text-neutral-500 hover:text-neutral-300 transition-colors">
